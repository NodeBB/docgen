...
function write(server, client) {
	var hookCount = os.EOL + os.EOL + '**Total Hooks: ' + hookCount + '**',
		date = os.EOL + '**This page was generated on ' + (new Date()).toUTCString();
		
	require('fs').writeFileSync('output/hooks.md', '# Server Hooks' + os.EOL + os.EOL + server + os.EOL + os.EOL + '# Client Hooks' + os.EOL + os.EOL + client + hookCount + date);
}
